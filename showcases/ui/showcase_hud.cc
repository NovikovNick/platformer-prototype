...
platformer::ShowcaseHUD::ShowcaseHUD(sf::RenderWindow& window)
    : selected_player_id_(0),
      player_select_labels_({"1st player", "2nd player"}) {
  ImGui::SFML::Init(window);
}

ShowcaseHUD::~ShowcaseHUD() { ImGui::SFML::Shutdown(); }

void ShowcaseHUD::handleEvent(sf::RenderWindow& window,
                              sf::Event& event) const {
  ImGui::SFML::ProcessEvent(window, event);
}

void ShowcaseHUD::draw(sf::RenderWindow& window, ShowcaseContext& ctx,
                       ShowcaseCallback& cb) {
  ImGui::SFML::Update(window, deltaClock.restart());

  ImGui::Begin("Network settings");
  ImGui::InputText("Your public ip:port", ctx.local_public_ip, 22,
